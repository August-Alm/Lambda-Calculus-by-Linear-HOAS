...
// Reference-counted strings. See "refcnt.sats".
...
extern fun{}
parse_term(input): context -<fun1> term

fun{}
parse_lam(inp: input): context -<fun1> term =
  let
    val name = s2rc(parse_name(inp))
    val ((*check*)) = parse_char('.', inp)
    val body = parse_term(inp)
...
fun{}
parse_app(inp: input): context -<fun1> term =
  let
    val func = parse_term(inp)
    val argm = parse_term(inp)
    val ((*check*)) = parse_char(')', inp)
  in
    lam(ctx) => App(func(incref(ctx)), argm(ctx))
  end
